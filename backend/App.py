...
RATE_LIMIT_POLICIES = {
    ...
    "/verify-code": 8,
    "/reset-password": 8,
    "/send-contact-email": 6,
    "/profile/upload-picture": 20,
    "/upload/file": 20,
    "/ai/": 20,
    "/gemini": 20,
    "/get_insight_data": 20,
...
def _rate_limit_for_path(path: str) -> int:
    for route_prefix, limit in RATE_LIMIT_POLICIES.items():
...
@app.middleware("http")
async def rate_limit_api_usage(request: Request, call_next):
    if request.method == "OPTIONS" or request.url.path.startswith("/uploads/files/"):
        return await call_next(request)

    limit = _rate_limit_for_path(request.url.path)
    key = f"api:{_client_identifier(request)}:{request.url.path}"
