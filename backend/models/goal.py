...
async def handle_goal(data: schema):
    ...
    goal_desc = data.goal_desc.strip()
    goal_time = data.goal_time.strip()
    ...
    if not goal or not goal_desc or not goal_time:
        raise HTTPException(status_code=400, detail="Goal, description, and time are required")
    ...
    try:
        ...
        return {
            "status": False,
            "message": f"Unable to save your goal: {exc}",
        }
    ...
    try:
        ...
        ai_response = {"success": False, "message": str(exc)}
