...
const Dashboard = () => {
  ...
  async function FetchingBackendData(event) {
    ...
    if (!goal.trim() || !goalDesc.trim() || !goal_time.trim()) {
      setStatusMessage('Enter a goal, description, and time before saving.')
      showToast('Complete all goal fields first.', 'warning')
      return
    }
    ...
    try {
      ...
      if (response.ok && data.status) {
        ...
        const message = data.detail || data.message || `Unable to save goal (HTTP ${response.status}).`
