...
function Profile() {
  ...
  async function handleSave(event) {
    ...
    try {
      ...
      setProfile(nextProfile)
      setFormData(nextProfile)
      const refreshedProfilePicture = nextProfile.profile_picture
        ? `${nextProfile.profile_picture}${nextProfile.profile_picture.includes('?') ? '&' : '?'}v=${Date.now()}`
        : ''
      setPreviewImage(refreshedProfilePicture)
      setSelectedFile(null)
      ...
      window.dispatchEvent(new CustomEvent('lifeos:profile-updated', {
        detail: { profile_picture: refreshedProfilePicture },
      }))
